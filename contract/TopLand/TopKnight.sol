...
interface ITopKnight {
    ...
    struct Version {
        ...
        uint startTime;
        uint revealTime;
        string provenance; // This is the provenance record of all TopKnight artworks in existence.
    }
     ...
     */
    function claimTopKnight(uint versionId, uint amount) external payable;
...
contract TopKnight is ITopKnight, ERC721, AcceptedToken, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    ) ERC721("TopKnight", "KNIGHT") AcceptedToken(tokenAddress) {
    ...
    modifier onlyKnightOwner(uint knightId) {
        require(ownerOf(knightId) == msg.sender, "TopKnight: not knight owner");
        _;
    }
    ...
    function claimTopKnight(uint versionId, uint amount) external override payable {
        Version storage version = versions[versionId];
        uint floorPrice = version.salePrice * 1000 / BPS;

        require(amount > 0 && amount <= 50, "TopKnight: amount out of range");
        require(block.timestamp >= version.startTime, "TopKnight: Sale has not started");
        require(version.currentSupply + amount <= version.maxSupply, "TopKnight: sold out");
        require(msg.value == version.salePrice * amount, "TopKnight: incorrect value");
    ...
    function changeKnightName(
        ...
        require(_validateStr(newName), "TopKnight: invalid name");
        require(reservedNames[newName] == false, "TopKnight: name already exists");
    ...
    function addFloorPriceToKnight(uint knightId) external override payable {
        ...
        require(msg.value > 0, "TopKnight: no value sent");
        require(newFloorPrice <= 100 ether, "TopKnight: cannot add more");
        require(acceptedToken.balanceOf(msg.sender) >= serviceFeeInToken, "TopKnight: insufficient token balance");
    ...
    function list(uint knightId, uint price) external override onlyKnightOwner(knightId) {
        require(price >= _knights[knightId].floorPrice, "TopKnight: under floor price");
    ...
    function delist(uint knightId) external override onlyKnightOwner(knightId) {
        require(knightsOnSale[knightId] > 0, "TopKnight: not listed");
    ...
    function buy(uint knightId) external override payable nonReentrant {
        ...
        require(price > 0, "TopKnight: not on sale");
        require(msg.value == price, "TopKnight: incorrect value");
        require(buyer != seller, "TopKnight: cannot buy your own Knight");
    ...
    function offer(uint knightId, uint offerValue) external override nonReentrant payable {
        ...
        require(buyer != ownerOf(knightId), "TopKnight: owner cannot offer");
        require(offerValue != currentOffer, "TopKnight: same offer");
        require(msg.value == requiredValue, "TopKnight: sent value incorrect");
    ...
    function takeOffer(
        ...
        require(offeredValue >= _knights[knightId].floorPrice, "TopKnight: under floor price");
        require(offeredValue >= minPrice, "TopKnight: less than min price");
        require(buyer != seller, "TopKnight: cannot buy your own Knight");
    ...
    function cancelOffer(uint knightId) external override nonReentrant {
        ...
        require(offerValue > 0, "TopKnight: no offer found");
    ...
    function learnSkill(uint knightId, uint skillId) external override onlyKnightOwner(knightId) {
        ...
        require(itemType == IEquipment.ItemType.SKILL_BOOK, "TopKnight: invalid skill book");

        bool isSuccess = skills.add(skillId);
        if (!isSuccess) revert("TopKnight: already learned");
    ...
    function adoptPet(uint knightId, uint petId) external override onlyKnightOwner(knightId) {
        require(petContract.ownerOf(petId) == msg.sender, "TopKnight: not pet owner");
    ...
    function abandonPet(uint knightId) external override onlyKnightOwner(knightId) {
        ...
        require(petId != 0, "TopKnight: no pet");
    ...
    function levelUp(uint knightId, uint amount) external override onlyOperator {
        ...
        require(amount > 0);
        require(newLevel <= maxLevel, "TopKnight: max level reached");
    ...
    function _setKnightEquipment(uint knightId, uint[] memory itemIds, bool isRemove) private {
        require(knightsOnSale[knightId] == 0, "TopKnight: cannot change items while on sale");
        require(itemIds.length > 0, "TopKnight: no item");
        ...
        for (uint i = 0; i < itemIds.length; i++) {
            ...
            require(itemId != 0, "TopKnight: invalid itemId");
            require(itemType != IEquipment.ItemType.SKILL_BOOK, "TopKnight: cannot equip skill book");
            require(!itemSet[uint(itemType)], "TopKnight: duplicate itemType");

            if (itemType == IEquipment.ItemType.MAIN_WEAPON) {
                require(isRemove ? knight.mainWeapon == itemId : knight.mainWeapon == 0, "TopKnight : invalid mainWeapon");
                ...
                require(isRemove ? knight.subWeapon == itemId : knight.subWeapon == 0, "TopKnight : invalid subWeapon");
